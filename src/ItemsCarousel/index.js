...
class ItemsCarousel extends React.Component {
  constructor(props) {
    ...
    this.state = {
      isPlaceholderMode:
        this.props.enablePlaceholder && this.props.children.length === 0,
    }
  }
  ...
  getItems = () => {
    const {placeholderItem, numberOfPlaceholderItems} = this.props

    const {isPlaceholderMode} = this.state
  ...
  renderList({items, translateX, containerWidth, measureRef}) {
    ...
    return (
      <Wrapper freeScrolling={freeScrolling}>
        <SliderItemsWrapper
          ref={measureRef}
          style={{
    ...
    return (
      <CarouselWrapper {...props}>
        <Motion
          ...
          children={({translateX}) =>
            this.renderList({items, measureRef, containerWidth, translateX})
          }
        />
        {_showRightChevron && (
...
ItemsCarousel.propTypes = {
   ...
   */
  rightChevron: PropTypes.oneOfType([PropTypes.element, PropTypes.string]),

  /**
   * Left chevron element. If passed `requestToChangeActive` must be set.
   */
  leftChevron: PropTypes.oneOfType([PropTypes.element, PropTypes.string]),
